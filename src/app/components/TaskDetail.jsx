...
const TaskDetail = ({
  ...
  <div clasName="card p-3 col-6">
    <div>
      <input onChange={setTaskName} value={task.name} className="form-control form-control-lg"/>
    </div>
    <div>
      <button className="btn btn-info mt-2" onClick={()=> setTaskCompletion(id, !isComplete)}>
...
const mapDispatchToProps = (dispatch, ownProps) => {
  ...
  return {
    ...
    setTaskGroup(e) {
      // e.target.value is the group that is selected
      dispatch(mutations.setTaskGroup(id, e.target.value))
    },
    setTaskName(e) {
      // e.target.value is the name that is selected
